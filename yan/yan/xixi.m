...
% Last Modified by GUIDE v2.5 13-Feb-2017 20:01:10
...
figure('NumberTitle', 'off', 'Name', '灰度直方图的波峰和波谷'); % 徐照波峰波谷
subplot(1,2,1);
[output_peak1 output_minima_low1 output_minima_high1] = findlocalminima(timgGray,10,10,0)
subplot(1,2,2);
[output_peak2 output_minima_low2 output_minima_high2] = findlocalminima(cimgGray,10,10,0)
if abs(output_peak2(1,2) - output_peak1(1,2)) / output_peak2(1,2) > 0.1
    if (output_peak2(1,2) - output_peak1(1,2)) > 0
        disp('笔墨偏浓');
    else
        disp('笔墨偏淡');
    end
